...
const _MAX_TOTAL_SIZE = 25 * 1024 * 1024; // 25MB
const _MAX_FILES_COUNT = 100; // Maximum number of files
  ...
  describe('NodeShipClient.deployments.upload()', () => {
    it('should call processFilesForNode for string[] input', async () => {
      ...
      fileUtilsMock.processFilesForNode.mockResolvedValueOnce([
        { path: 'file.txt', content: Buffer.from('content'), md5: 'm', size: 1 },
      ]);
    ...
    it('should never use basePath for prefixing uploaded paths', async () => {
      ...
      await client.deployments.upload(['/base/folder/file1.txt', '/base/folder/nested/file2.txt'], {
        pathDetect: true,
      } as DeploymentOptions);
      // The returned paths should be root-relative, not prefixed with basePath
      expect((apiClientMock.deploy.mock.calls[0][0] as any[]).map((f) => f.path)).toEqual([
        'file1.txt',
        'nested/file2.txt',
      ]);
    });
    ...
    it('should throw ShipError for File[] input', async () => {
      ...
      await expect(client.deployments.upload([fakeFile] as any, {})).rejects.toThrow(
        ShipError.business(
    ...
    it('should throw ShipError for FileList input', async () => {
      ...
      const fakeFileList = {
        0: { name: 'f.txt', size: 1, type: 'text/plain' },
        length: 1,
        item: () => null,
      };
      await expect(client.deployments.upload(fakeFileList as any, {})).rejects.toThrow(
        ShipError.business(
          'Invalid input type for Node.js environment. Expected string or string[].',
        ),
      );
    ...
    it('should throw ShipError for HTMLInputElement input', async () => {
      ...
      await expect(client.deployments.upload(fakeInput as any, {})).rejects.toThrow(
        ShipError.business(
          'Invalid input type for Node.js environment. Expected string or string[].',
        ),
      );
    ...
    it('should throw ShipError for Buffer input', async () => {
      ...
      await expect(client.deployments.upload(fakeBuffer as any, {})).rejects.toThrow(
        ShipError.business(
          'Invalid input type for Node.js environment. Expected string or string[].',
        ),
      );
    ...
    it('should exclude empty files during processing', async () => {
      ...
      fileUtilsMock.processFilesForNode.mockImplementationOnce((_paths, _options) => {
      ...
      expect(apiClientMock.deploy).toHaveBeenCalledWith(
        expect.arrayContaining([expect.objectContaining({ path: 'file.txt' })]),
        expect.anything(),
      );
