...
describe('Ship SDK Events - Final Demo', () => {
  test('should demonstrate impossible simplicity - 3 events, 2 methods', async () => {
    ...
    const pingRequest = apiCalls.find(
      (call) => call.type === 'request' && call.url.includes('/ping'),
    );
    const pingResponse = apiCalls.find(
      (call) => call.type === 'response' && call.url.includes('/ping'),
    );
