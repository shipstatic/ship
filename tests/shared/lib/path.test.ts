...
function _mockFileList(files: File[]): FileList {
...
describe('Strip Common Prefix Test', () => {
  ...
  describe('Ship.upload with flattenDirs', () => {
    it('should correctly strip nested parent folders', async () => {
      ...
      const allPathsStripped = uploadedFiles.every(
        (f) => !f.path.startsWith('fdsa') && !f.path.startsWith('asdf/'),
      );
    ...
    it('should respect flattenDirs for nested folders', async () => {
      ...
      const allPathsCorrect = uploadedFiles.every(
        (f) => !f.path.includes('fdsa') && !f.path.includes('asdf/'),
      );
