...
describe('Concurrent Operations', () => {
  ...
  describe('Concurrent API calls', () => {
    ...
    it('should handle concurrent domain operations', async () => {
      ...
      const results = await Promise.all([
        ship.domains.set('concurrent-1', { deployment: 'test-deployment-1.shipstatic.com' }),
        ship.domains.set('concurrent-2', { deployment: 'test-deployment-1.shipstatic.com' }),
        ship.domains.set('concurrent-3', { deployment: 'test-deployment-1.shipstatic.com' }),
      ]);
