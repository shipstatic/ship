    ...
    return {
      async shouldRejectStringPaths() {
        await expect(ship.deploy('/path/to/file' as any)).rejects.toThrow(
          'Invalid input type for browser environment',
        );
      },
  ...
  async testBrowserSpecificErrors() {
    ...
    return {
      ...
      async shouldHandleNullFiles() {
        ...
        await expect(ship.deploy(fileListWithNull)).rejects.toThrow();
      },
    };
  },
};
    ...
    return {
      async shouldRejectFileArrays() {
        ...
        await expect(ship.deploy(mockFiles)).rejects.toThrow(
          'Invalid input type for Node.js environment',
        );
      },
  ...
  async testNodeSpecificErrors() {
    ...
    return {
      async shouldHandleNonExistentPaths() {
        await expect(ship.deploy(['/non/existent/path'])).rejects.toThrow();
      },
      ...
      async shouldHandleInvalidPathTypes() {
        await expect(ship.deploy([null, undefined, 123] as any)).rejects.toThrow();
      },
    };
  },
};
